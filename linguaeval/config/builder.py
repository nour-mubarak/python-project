...
VALID_SECTORS = ["government", "university", "finance", "healthcare", "legal", "general"]
VALID_DIMENSIONS = ["accuracy", "bias", "hallucination", "consistency", "cultural", "fluency"]
VALID_MODELS = [
...
class EvaluationConfig:
    ...
    # Models to evaluate
    models: List[str] = field(default_factory=lambda: ["gpt-4o", "claude-sonnet-4-20250514"])
    ...
    def get_model_configs(self) -> List[ModelConfig]:
        ...
        for model_id in self.models:
            if model_id.startswith("gpt") or model_id.startswith("azure"):
                ...
                configs.append(ModelConfig(
                    model_id=model_id,
                    display_name=model_id.split("-")[0].title() + " " + model_id.split("-")[1].title(),
                    provider="anthropic",
                    temperature=0.3,
                ))
            else:
