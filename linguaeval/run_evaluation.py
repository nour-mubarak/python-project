...
from config.builder import EvaluationConfig, preset_university, preset_government, preset_finance
...
def load_prompt_pack(sector: str) -> list:
    ...
    if not os.path.exists(prompt_file):
        ...
        print(f"    Available packs: {', '.join(f.replace('.json','') for f in os.listdir(os.path.join(os.path.dirname(__file__), 'prompts')) if f.endswith('.json'))}")
        sys.exit(1)
...
def run_evaluation(config: EvaluationConfig, use_judge: bool = False, dry_run: bool = False):
    """
    ...
    raw_path = os.path.join(config.output_dir, f"{config.client_name.lower().replace(' ', '_')}_{timestamp}_raw.json")
    ...
    if use_judge:
        ...
        judge_config = ModelConfig(
            ...
            display_name="Judge",
            provider="anthropic" if "claude" in config.judge_model else "openai",
            temperature=0.1,
            max_tokens=500,
        )
    ...
    for prompt_id, models_data in response_map.items():
        ...
        for model_id, lang_responses in models_data.items():
            ...
            for lang, response_text in lang_responses.items():
                scoring_count += 1
                print(f"\r  Scoring {scoring_count}/{total_scoring}: {model_id} | {lang.upper()} | {prompt_id}", end="", flush=True)
    ...
    aggregates = calculate_aggregates(all_results, [mc.model_id for mc in model_configs])
        ...
        for dim in agg[model_id]["en"]:
            if dim in agg[model_id]["ar"]:
                ...
                agg[model_id]["cross_lingual_gap"][dim] = {
                    ...
                    "en_avg": en_avg,
                    "ar_avg": ar_avg,
                    "severity": "critical" if gap > 20 else "high" if gap > 12 else "medium" if gap > 6 else "low",
                }
...
def print_summary(aggregates: dict):
    ...
    for model_id, data in aggregates.items():
        ...
        for dim in data.get("en", {}):
            ...
            en_str = f"{en_score:>5.1f}%" if isinstance(en_score, (int, float)) else f"{en_score:>6}"
            ar_str = f"{ar_score:>5.1f}%" if isinstance(ar_score, (int, float)) else f"{ar_score:>6}"
            gap_str = f"{gap:>5.1f}%" if isinstance(gap, (int, float)) else f"{gap:>6}"
            
            print(f"    {dim:<25} EN: {en_str}  AR: {ar_str}  Gap: {gap_str}  [{severity}]")
...
def main():
    ...
    parser.add_argument("--preset", type=str, choices=["university", "government", "finance"],
                        help="Use a preset configuration")
    parser.add_argument("--client", type=str, default="Test Client", help="Client name (with --preset)")
    parser.add_argument("--use-judge", action="store_true", help="Use judge model for enhanced scoring")
    ...
    if args.config:
        ...
        presets = {"university": preset_university, "government": preset_government, "finance": preset_finance}
        config = presets[args.preset]()
        config.client_name = args.client
    else:
        print("Usage: python run_evaluation.py --config <path> OR --preset <type>")
        print("       python run_evaluation.py --preset government --client 'SDAIA' --dry-run")
        sys.exit(1)
